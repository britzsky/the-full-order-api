...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='식단표 공통 식사구분 코드';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='고객사별 식단표 제공 식사구분 설정';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='식자재 공급처 마스터';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='공급처별 식재료 판매상품. 공급처가 다르면 같은 식재료도 별도 행';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='공급처 상품 구매가격 이력';
...
CREATE TABLE tb_account_ingredient_master (
    ...
    CONSTRAINT fk_account_ingredient_standard
        FOREIGN KEY (ingredient_id) REFERENCES tb_ingredient_master (ingredient_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='거래처별 표준 식재료 정보. 공급처 상품·가격·재고는 별도 테이블에서 관리';
...
CREATE TABLE tb_account_ingredient_product (
    ...
    CONSTRAINT fk_account_product_supplier_product
        FOREIGN KEY (supplier_product_id)
        REFERENCES tb_supplier_ingredient_product (supplier_product_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='거래처가 사용하는 공급처별 식재료 상품';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='공통 메뉴 마스터';
...
CREATE TABLE tb_recipe_detail (
    ...
    CONSTRAINT fk_recipe_ingredient
        FOREIGN KEY (ingredient_id) REFERENCES tb_ingredient_master (ingredient_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='메뉴 마스터의 표준 1인분 기준 레시피 식재료';
...
CREATE TABLE tb_account_menu_master (
    ...
    CONSTRAINT fk_account_menu_standard
        FOREIGN KEY (menu_id) REFERENCES tb_menu_master (menu_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='거래처가 사용하도록 등록한 메뉴';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='거래처 메뉴의 1인분 기준 정규화 레시피 식재료';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='거래처 식단표 헤더';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='식단표 날짜·끼니별 식수와 1인 식단가';
...
CREATE TABLE tb_account_table_meals_detail (
    ...
    CONSTRAINT fk_meal_detail_account_menu
        FOREIGN KEY (account_id, menu_id)
        REFERENCES tb_account_menu_master (account_id, menu_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='끼니별 메뉴 구성';
...
CREATE TABLE tb_account_meal_menu_cost_snapshot (
    ...
    CONSTRAINT fk_meal_menu_cost_service
        FOREIGN KEY (meal_service_id) REFERENCES tb_account_meal_service (meal_service_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='식단 생성·확정 당시 메뉴 원가 스냅샷';
...
CREATE TABLE tb_account_meal_ingredient_requirement (
    ...
    CONSTRAINT fk_requirement_product
        FOREIGN KEY (supplier_product_id)
        REFERENCES tb_supplier_ingredient_product (supplier_product_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='식단 확정 당시 메뉴별 식재료 필요량과 원가 근거';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='거래처·공급처상품·보관위치별 현재 재고';
...
CREATE TABLE tb_account_inventory_movement (
    ...
    CONSTRAINT fk_movement_account_product
        FOREIGN KEY (account_ingredient_product_id)
        REFERENCES tb_account_ingredient_product (account_ingredient_product_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='실제 재고 입고·사용·조정 이력. 평균 실제 사용량 계산 근거';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='식단 또는 재고관리에서 생성한 외부발주 전 대기 목록';
...
CREATE TABLE tb_account_procurement_cart_item (
    ...
    CONSTRAINT fk_cart_item_account_product
        FOREIGN KEY (account_ingredient_product_id)
        REFERENCES tb_account_ingredient_product (account_ingredient_product_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='발주 대기 목록의 공급처별 상품';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='거래처의 공급처별 실제 발주 헤더';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='실제 발주의 공급처 상품별 품목';
